...
Result<ImageUniquePtr> loadImageImpl(const String & _path, Allocator & _alloc)
{
...
Error saveImageImpl(const Image & _image, const String & _path)
{
    ByteArray data;
    ...
    if (fif != FIF_UNKNOWN)
    {
        ...
        if (_image.channelLayoutTypeID() == ChannelLayoutRGB::TypeInfo::typeID() ||
            _image.channelLayoutTypeID() == ChannelLayoutRGBA::TypeInfo::typeID())
        {
            tmp = _image.clone();
